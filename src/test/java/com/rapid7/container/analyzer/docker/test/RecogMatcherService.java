...
public class RecogMatcherService {

  private RecogMatchersProvider provider;

  public RecogMatcherService(File recogContentDirectory) {
    provider = new RecogMatchersProvider(RecogType.BUILTIN, recogContentDirectory);
  }

  public List<RecogMatchResult> fingerprint(String description) {
    List<RecogMatchResult> matches = new ArrayList<>();
    matches.addAll(fingerprint(description, provider.getMatchers(RecogType.BUILTIN)));
